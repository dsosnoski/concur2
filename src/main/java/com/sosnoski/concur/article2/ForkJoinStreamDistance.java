 ...
 */
public class ForkJoinStreamDistance extends TimingTestBase
{
    ...
    public class DistanceTask extends RecursiveTask<DistancePair>
    {
         ...
         */
        private int editDistance(String known, int[] v0, int[] v1) {
            ...
            for (int i = 0; i < known.length(); i++) {
                ...
                for (int j = 0; j < compareText.length(); j++) {
                    int cost = (known.charAt(i) == compareText.charAt(j)) ? 0 : 1;
        ...
        protected DistancePair compute() {
			...
			return IntStream.range(startOffset, startOffset + compareCount)
			    .mapToObj((i) -> new DistancePair(editDistance(knownWords[i], v0, v1), knownWords[i]))
